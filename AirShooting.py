...
pygame.display.set_caption('Air Shooting')
...
HEALSPAWN = pygame.USEREVENT + 3
pygame.time.set_timer(HEALSPAWN, 7000)

SENEMYSPAWN = pygame.USEREVENT + 5
pygame.time.set_timer(SENEMYSPAWN, random.randint(4000,6000))
...
bossmusic = pygame.mixer.Sound('./audio/boss.wav')
bossmusic.set_volume(.7)
...
class Healpowerup(pygame.sprite.Sprite):
    def __init__(self, direction, speed):
        super().__init__()
        self.image = pygame.image.load('./images/healPowerUp.png').convert_alpha()
        self.rect = self.image.get_rect()
        self.direction = direction
        self.speed = speed
...
def gameover(won=False):
    ...
    EMBERSPAWN = pygame.USEREVENT + 4
    pygame.time.set_timer(EMBERSPAWN,100)

    gameoverimg = pygame.image.load('./images/died.png')
    gameoverimg = Image(gameoverimg,(screen_w/2,screen_h/2))
    if(won):
        gameoverimg = pygame.image.load('./images/won.png')
        gameoverimg = Image(gameoverimg, (screen_w / 2, screen_h/2))
    ...
    while over:
        ...
        s_rect = s.get_rect()
        s_rect.center = (screen_w/2,screen_h/2+100)
...
def game():
    ...
    global player,blockspeed1,blockspeed2,barHP,upgrade,score,upgradescore,increment,increment,gametrack,music_change,muted, bossstage
    pygame.event.set_allowed([ENEMYSPAWN,SENEMYSPAWN])
    all_blocks.empty()
...
def innergame():
    ...
    global player,blockspeed1,blockspeed2,barHP,upgrade,score,upgradescore,increment,bossstage,b1x,b1y,b2x,b2y,c1x,c2x,c3x,c4x,c1y,c2y,c3y,c4y ,gametrack,music_change,muted
    ...
    while not crash:
        ...
        for event in pygame.event.get():
            ...
            if event.type == HEALSPAWN:
                healpowerup = Healpowerup(1,10)
                healpowerup.rect.centerx = random.randrange(screen_w)
                healpowerup.rect.centery = 0
                all_sprites.add(healpowerup)
                all_powerup.add(healpowerup)


            if event.type == SENEMYSPAWN:
                blockspecial = Block(3, random.randint(blockspeed1, blockspeed2))
                blockspecial.image = pygame.image.load('./images/e_jet2.png').convert_alpha()
                blockspecial.rect = blockspecial.image.get_rect()
                blockspecial.radius = (blockspecial.rect.centerx - blockspecial.rect.x)
                blockspecial.hp = 6
                blockspecial.type = 1
                all_blocks.add(blockspecial)
                all_sprites.add(blockspecial)
                blockspecial.rect.centerx = random.randrange(screen_w)
                blockspecial.rect.centery = random.randint(-140, -90)

            if event.type == ENEMYSPAWN:
                block = Block(3,random.randint(blockspeed1,blockspeed2))
                if score>=2 and not bossstage:
                    bossstage = True
                    pygame.event.set_blocked([SENEMYSPAWN])
                ...
                if block.type == 2:
                    block.rect.centerx = (screen_w/2)
                    block.rect.centery = 130
        ...
        for powerups in all_powerup:
            powerup_hit_list = pygame.sprite.spritecollide(player, all_powerup, False,
